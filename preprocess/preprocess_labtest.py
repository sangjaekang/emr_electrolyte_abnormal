...
def get_timeserial_lab_df(no):
    ...
    try:
        ...
        lab_store.close()
    _y = target_df[['lab_test', 'date', 'result']].pivot_table(
        index=['lab_test'], columns=['date'])
...
def preprocess_labtest():
    ...
    if DEBUG_PRINT:
        print("preprocess_labtest starts")
    if not os.path.isfile(RAW_LABTEST_PATH):
        raise ValueError(' there is no RAW_LABTEST file (wrong path)')

    lab_df = pd.read_csv(RAW_LABTEST_PATH, delimiter=DELIM,
                         header=None, names=RAW_LABTEST_COLS)
    # 이전 전처리된 파일이 있으면 삭제
    ...
    lab_df.loc[:, 'date'] = pd.to_datetime(
        lab_df['date'].astype(str), format='%Y%m%d')

    no_lab_times = lab_df[['no', 'date']].drop_duplicates()
    # 검사를 한번만（하루만） 받은 사람들 제거
    no_once = no_lab_times.groupby('no').count(
    )[no_lab_times.groupby('no').count() > 1].date
    ...
    no_exist = test_interval[test_interval >
                             np.timedelta64(SKIP_TEST_INTERVAL, 'D')].index
    ...
    orginal_lab_df.to_hdf(LABTEST_PATH, 'original',
    ...
    for lab_name in pre_lab_df.lab_test.unique():
        ...
        if DEBUG_PRINT:
            print("    normalize {} ends".format(lab_name))
    ...
    count_lab_df = normalized_lab_df[
        ['no', 'lab_test']].groupby('lab_test').count()
...
def set_mapping_table(lab_df):
    ...
    if DEBUG_PRINT:
        print("set_mapping_table starts")
